...
class Experiment():
    ...
    def run(self):
        ...
        print(f"Experiment Acc: {np.mean(experiment_acc_history)*100: 0.2f}",
              u"\u00B1",
              f"{np.std(experiment_acc_history)*100: 0.2f}")

        with open(f"{cfg.outdir_path}/result.txt",'w') as f:
            f.write(f"Experiment Acc: {np.mean(experiment_acc_history)*100: 0.2f}")
            f.write(u"\u00B1")
            f.write(f"{np.std(experiment_acc_history)*100: 0.2f}")
